    ...
    for (i = 0; i < 10; i++) {
        ...
        if (errno != EINTR && errno != EACCES) {
            ...
            if (current_time.tv_sec >= start_time.tv_sec &&
                ...
                ALOGE("Failed to open device. (err=%d)", -errno);
                return -errno;
            }
        }
    }
   ...
   int carrier_freq, int pattern[], int pattern_len)
{
    ...
    ALOGD("Consumer IR HAL is transmitting - Type : %d", g_sol);
        ...
        hw_device_t **device)
{
    ...
    if (g_sol < 0) {
        ...
        ALOGD("Trying to use F Chip.");

        fd = open(FPGA_IR_PATH, O_RDWR);
        if (fd < 0)
            ALOGE("Consumer IR HAL has failed to load (F Chip)");

        g_sol = 0;

        if (close(fd) < 0)
            ALOGE("Consumer IR HAL has failed to close (F Chip)");
    }

    if (g_sol < 0)
        ALOGE("Consumer IR Hal has failed to load");

    ALOGD("Consumer IR HAL is loading [Type : %s] [SOL : %d]", "TIME", g_sol);
