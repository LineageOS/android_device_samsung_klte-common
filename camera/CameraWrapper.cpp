...
#define KEY_QC_ZSL_HDR_SUPPORTED "zsl-hdr-supported"
...
static char *camera_fixup_getparams(int id __unused, const char *settings)
{
    ...
    params.set(KEY_QC_ZSL_HDR_SUPPORTED, "true");
...
static char *camera_fixup_setparams(int id, const char *settings)
{
    ...
    if (sceneMode) {
        if (id == REAR_CAMERA_ID && !strcmp(sceneMode, CameraParameters::SCENE_MODE_HDR)) {
...
static int camera_start_recording(struct camera_device *device)
{
    ...
    if (is_4k_video(parameters)) {
        parameters.setPreviewFormat(QC_PIXEL_FORMAT_NV12_VENUS);
